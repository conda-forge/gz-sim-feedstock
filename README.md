...
Once the `conda-forge` channel has been enabled, `gz-sim, gz-sim-python, libgz-sim` can be installed with `conda`:

```
conda install gz-sim gz-sim-python libgz-sim
```

or with `mamba`:

```
mamba install gz-sim gz-sim-python libgz-sim
```
